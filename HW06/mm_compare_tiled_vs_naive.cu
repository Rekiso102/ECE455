...
template <typename T> std::vector<T> create_rand_vector(size_t n, T min_val = 0, T max_val = 10) {
    std::random_device r;
...
template <typename T> void mm_host(const T* A, const T* B, T* C, int N) {
    for (int i = 0; i < N; ++i) {
        for (int j = 0; j < N; ++j) {
            double acc = 0.0;
            for (int k = 0; k < N; ++k)
...
template <typename T> __global__ void mm_naive(const T* A, const T* B, T* C, int N) {
...
template <typename T> __global__ void mm_tiled(const T* A, const T* B, T* C, int N) {
...
template <typename T> bool validate_results(const std::vector<T>& ref, const std::vector<T>& gpu, int N, T rel_tol = static_cast<T>(1e-2)) {
    for (int i = 0; i < N; ++i) {
        for (int j = 0; j < N; ++j) {
            size_t idx = i * N + j;
...
// ---------------- CUDA Event Timing ----------------
template <typename KernelFunc, typename T> float measure_kernel_time(KernelFunc kernel, int N, bool tiled) {
